...
const sVersion_CodeType ErrorAlarm_VerDef =
{
    MODULE_ERRORALARM_NAME,
    ERRORALARM_SW_MAJOR_VERSION,
    ERRORALARM_SW_MINOR_VERSION,
    ERRORALARM_SW_REVISION_VERSION,
    0,
    0,
    ERRORALARM_DEVELOP_STATUS
};

/* Version Get Interface of Version Module */
ModuleID_Type ErrorAlarm_GetVesionModuleVer(sVersion_CodeType * VerPtr)
{
    *VerPtr = ErrorAlarm_VerDef;
    return MODULE_ERRORALARM_ID;
}
...
typedef struct
{
    ...
    sErr_HystrsType DeltV[ERR_LEVEL_NUM];
    sErr_HystrsType THigh[ERR_LEVEL_NUM];
    sErr_HystrsType TLow[ERR_LEVEL_NUM];
    sErr_HystrsType DeltT[ERR_LEVEL_NUM];
...
uErr_HwErrCodeType Err_HwErrCodeBmu[MAX_PACK_NUM];

Err_HwErrDetailType Err_HwErrDetailBmu[MAX_PACK_NUM][ERR_NUM_HW_DETAIL];
...
static void Err_DealHwErrDetail(void);
...
void Err_Main(void)
{
    ...
    if(ERR_FLAG_FAIL == gby_ReadErrThrhd)
    {
...
GeneralReturnType Err_JudgeBatErrLevel(void)
{
    ...
    Err_JudgeHighThrhd(Bat_GetMaxT(), Err_HystrsThrhd, Err_HystrsCnt.THigh,
                       max_cnt_value, &err_level);
    ...
    /* battery manu diff */
#if(ECU_CATEGORY == M_BCU_C)
    bat_manu = Bat_GetPackManufacturer(0);
    for(i=1; i<MAX_PACK_NUM; i++)
#elif(ECU_CATEGORY == M_BCU_D)
    ...
    /* battery type diff */
#if(ECU_CATEGORY == M_BCU_C)
    bat_type = Bat_GetPackMaterialType(0);
    for(i=1; i<MAX_PACK_NUM; i++)
#elif(ECU_CATEGORY == M_BCU_D)
    ...
    err_code = SysConf_GetParaValueFromRam(SYSCONF_BMU_NO, &bmu_no, &length);
    ...
    err_level = Err_BatErrCode.sBit.CellVHigh;
    Err_JudgeHighThrhd(Bat_GetPackMaxV(bmu_no), Err_HystrsThrhd, Err_HystrsCnt.CellVHigh,
                       max_cnt_value, &err_level);
    Err_BatErrCode.sBit.CellVHigh = err_level;
    ...
    err_level = Err_BatErrCode.sBit.CellVLow;
    Err_JudgeLowThrhd(Bat_GetPackMinV(bmu_no), Err_HystrsThrhd, Err_HystrsCnt.CellVLow,
                      max_cnt_value, &err_level);
    Err_BatErrCode.sBit.CellVLow = err_level;
    ...
    Err_HystrsThrhd[0].Error = Err_Thrhd[0].DeltV;
    Err_HystrsThrhd[0].Normal = Err_Thrhd[0].DeltV - ERR_THRHD_HYSTRS_DELTV_L1;
    Err_HystrsThrhd[1].Error = Err_Thrhd[1].DeltV;
    Err_HystrsThrhd[1].Normal = Err_Thrhd[1].DeltV - ERR_THRHD_HYSTRS_DELTV_L2;
    Err_HystrsThrhd[2].Error = Err_Thrhd[2].DeltV;
    Err_HystrsThrhd[2].Normal = Err_Thrhd[2].DeltV - ERR_THRHD_HYSTRS_DELTV_L3;

    err_level = Err_BatErrCode.sBit.DeltVSinglPack;
    Err_JudgeHighThrhd(Bat_GetPackDeltaV(bmu_no), Err_HystrsThrhd, Err_HystrsCnt.DeltV,
                       max_cnt_value, &err_level);
    Err_BatErrCode.sBit.DeltVSinglPack = err_level;
    ...
    err_level = Err_BatErrCode.sBit.THigh;
    Err_JudgeHighThrhd(Bat_GetPackMaxT(bmu_no), Err_HystrsThrhd, Err_HystrsCnt.THigh,
                       max_cnt_value, &err_level);
    Err_BatErrCode.sBit.THigh = err_level;
    ...
    err_level = Err_BatErrCode.sBit.TLow;
    Err_JudgeLowThrhd(Bat_GetPackMinT(bmu_no), Err_HystrsThrhd, Err_HystrsCnt.TLow,
                      max_cnt_value, &err_level);
    Err_BatErrCode.sBit.TLow = err_level;
    ...
    Err_HystrsThrhd[0].Error = Err_Thrhd[0].DeltT;
    Err_HystrsThrhd[0].Normal = Err_Thrhd[0].DeltT - ERR_THRHD_HYSTRS_DELTT_L1;
    Err_HystrsThrhd[1].Error = Err_Thrhd[1].DeltT;
    Err_HystrsThrhd[1].Normal = Err_Thrhd[1].DeltT - ERR_THRHD_HYSTRS_DELTT_L2;
    Err_HystrsThrhd[2].Error = Err_Thrhd[2].DeltT;
    Err_HystrsThrhd[2].Normal = Err_Thrhd[2].DeltT - ERR_THRHD_HYSTRS_DELTT_L3;

    err_level = Err_BatErrCode.sBit.DeltTSinglPack;
    Err_JudgeHighThrhd(Bat_GetPackDeltaT(bmu_no), Err_HystrsThrhd, Err_HystrsCnt.DeltT,
                       max_cnt_value, &err_level);
    Err_BatErrCode.sBit.DeltTSinglPack = err_level;
    ...
    err_level = Err_BatErrCode.sBit.SocLow;
    Err_JudgeLowThrhd(Bat_GetPackSoc(0), Err_HystrsThrhd, Err_HystrsCnt.SocLow,
                      max_cnt_value, &err_level);
    Err_BatErrCode.sBit.SocLow = err_level;

    /* charge current high */
    current = Bat_GetPackCurrent(0, INSCUR);
    ...
    //no different between chg or feedback
    if(CHG_ST_ON == chg_st)
    {
        max_cnt_value[0] = ERR_CNT_SET_CURCHG_L1;
        max_cnt_value[1] = ERR_CNT_CLR_CURCHG_L1;
