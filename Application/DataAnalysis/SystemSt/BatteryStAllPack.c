...
const sVersion_CodeType SystemSt_VerDef =
{
    MODULE_SYSTEMST_NAME,
    SYSTEMST_SW_MAJOR_VERSION,
    SYSTEMST_SW_MINOR_VERSION,
    SYSTEMST_SW_REVISION_VERSION,
    0,
    0,
    
    SYSTEMST_DEVELOP_STATUS
};

/* Version Get Interface of Version Module  */
ModuleID_Type SystemSt_GetVesionModuleVer(sVersion_CodeType * VerPtr)
{      
    *VerPtr = SystemSt_VerDef;
    return MODULE_SYSTEMST_ID;   
}
...
/* if ecu have measure function, define for local information */
/* Ecu include BCU_C BMU BMU_R */
sBat_CellBasicType  Bat_LocalCellBasicBuf[MAX_PACK_CELL_NUM];
sBat_CellStType     Bat_LocalCellStBuf[MAX_PACK_CELL_NUM];
sBat_PackTemperType  Bat_LocalPackTemperBuf[MAX_PACK_TEMPER_NUM];



sBat_PackInfoType  Bat_LocalPackInfo;
#endif  /* #if ((ECU_CATEGORY == M_BCU_C) || (ECU_CATEGORY == M_BMU) || (ECU_CATEGORY == M_BMU_R)) */

#if ((ECU_CATEGORY == M_BMU) || (ECU_CATEGORY == M_BMU_R))

sBat_PackInfoType *  Bat_AllPackInfo[] =
{
    &Bat_LocalPackInfo,
};
...
#if ((ECU_CATEGORY == M_BCU_C) || (ECU_CATEGORY == M_BMU) || (ECU_CATEGORY == M_BMU_R))
    /* initial local Pack information */
    BatPackInfo_Init(&Bat_LocalPackInfo, Bat_LocalCellBasicBuf,Bat_LocalCellStBuf,Bat_LocalPackTemperBuf);
#endif
...
#if ((ECU_CATEGORY == M_BMU) || (ECU_CATEGORY == M_BMU_R))
    return Bat_AllInfo.PackInfoPtrRef[0]->PackTemperPtr[local_temper_no].Temper;
#endif
...
#if ((ECU_CATEGORY == M_BMU) || (ECU_CATEGORY == M_BMU_R))
    Bat_AllInfo.PackInfoPtrRef[0]->PackTemperPtr[local_temper_no].Temper = temper;
#endif
