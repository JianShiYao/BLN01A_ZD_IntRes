...
void GbChg_CountCbk(void)
{
   ...
   /* (GbChgCan_TmCnt.Bro < 255) ? (GbChgCan_TmCnt.Bro++) : (GbChgCan_TmCnt.Bro = 255); */
    /*(GbChgCan_TmCnt.Bcl < 255) ? (GbChgCan_TmCnt.Bcl++) : (GbChgCan_TmCnt.Bcl = 255); */
    (GbChgCan_TmCnt.Bcs < 255) ? (GbChgCan_TmCnt.Bcs++) : (GbChgCan_TmCnt.Bcs = 255);
    /*(GbChgCan_TmCnt.Bsm < 255) ? (GbChgCan_TmCnt.Bsm++) : (GbChgCan_TmCnt.Bsm = 255); */
    (GbChgCan_TmCnt.Bmv < 65535) ? (GbChgCan_TmCnt.Bmv++) : (GbChgCan_TmCnt.Bmv = 65535);
    (GbChgCan_TmCnt.Bmt < 65535) ? (GbChgCan_TmCnt.Bmt++) : (GbChgCan_TmCnt.Bmt = 65535);
    (GbChgCan_TmCnt.Bsp < 255) ? (GbChgCan_TmCnt.Bsp++) : (GbChgCan_TmCnt.Bsp = 255);
  /*  (GbChgCan_TmCnt.Bst < 255) ? (GbChgCan_TmCnt.Bst++) : (GbChgCan_TmCnt.Bst = 255); */
  /*  (GbChgCan_TmCnt.Bsd < 255) ? (GbChgCan_TmCnt.Bsd++) : (GbChgCan_TmCnt.Bsd = 255); */
  /*  (GbChgCan_TmCnt.Bem < 255) ? (GbChgCan_TmCnt.Bem++) : (GbChgCan_TmCnt.Bem = 255); */
   ...
   // (GbChgCan_TmCnt.RxChgMsg < 65535) ? (GbChgCan_TmCnt.RxChgMsg++) : (GbChgCan_TmCnt.RxChgMsg = 65535);
...
static void GbChg_TpcmSendData(uByte pf, uByte msg_no)
{
    ...
    case BCP_PF:
    {
        if(1 == msg_no)
        {
            ...
            /* max charge current  */
            if(Chg_GetPermitMaxCurrent() < 4000)
            {
               wtmp = 4000 - Chg_GetPermitMaxCurrent();
            }
            else
            {
               wtmp = 0;
    ...
    case BCS_PF:
    {
        if(1 == msg_no)
        {
           ...
           /* sumvolt  0.1V unit */
            ...
            wtmp = 4000 - wtmp; /* 0.1A unit and -400A offset*/
            msg.data[3] = (uByte)(wtmp & 0x00ff);
            msg.data[4] = (uByte)((wtmp & 0xff00) >> 8);
...
void GbChgCan_TpcmProc(void)
{
    ...
    if( (pGbChgCnt->Brm >= TM_MS(250))
            && (GB_FLAG_ENABLE == GbChgM_GetFlgBrm())//GbChgM_Ctrl.Flg.Brm == GB_FLAG_ENABLE)  /* send message Brm enable */
            ...
            && (pGbChgCanCtrl->Flg.RxCtsBrm == GB_FLAG_FAIL) )	 /*has Not receive cts of Brm */
    {
        ...
        GbChg_TpcmRts(RTS, gby_byte_num_brm, gby_msg_num_brm, BRM_PF);		   /* request send Brm  */
    }

    if( (pGbChgCnt->Bcp>= TM_MS(500))
            && (GB_FLAG_ENABLE == GbChgM_GetFlgBcp())  /* send message Bcp enable */
            && (pGbChgCanCtrl->Tpcm == TPCM_NULL)     /* use Tpcm enable */
    ...
    if( (pGbChgCnt->Bcs >= TM_MS(250))
            && (GB_FLAG_ENABLE == GbChgM_GetFlgBcs())  /* send message Bcs enable */
            && (pGbChgCanCtrl->Tpcm == TPCM_NULL)    /*  use Tpcm enable */   
    ...
    if( (pGbChgCnt->Bmv >= TM_MS(10000))
            && (GB_FLAG_ENABLE == GbChgM_GetFlgBmv()) /* send message Bmv enable */
            && (pGbChgCanCtrl->Tpcm == TPCM_NULL)    /* use Tpcm enable  */
    ...
    if( (pGbChgCnt->Bmt>= TM_MS(10000))
            && (GB_FLAG_ENABLE == GbChgM_GetFlgBmt()) /* send message Bmt enable */
            && (pGbChgCanCtrl->Tpcm == TPCM_NULL)    /* use Tpcm enable  */
    ...
    if( (pGbChgCnt->Bsp >= TM_MS(1000))
            && (GB_FLAG_ENABLE == GbChgM_GetFlgBsp()) /* send message Bsp enable */
            && (pGbChgCanCtrl->Tpcm == TPCM_NULL)    /* use Tpcm enable */
...
void GbChgCan_LoadData(void)
{
   ...
   /* use Tpcm */
    ...
    /* battery status */
    GbChgCan_Msg[2].Msg.data[5] =  ChgM_GetBatSt();
    ...
    /* Bst */
    GbChgCan_Msg[3].Msg.data[0] = GbChgM_GetChgEndReason1();

    GbChgCan_Msg[3].Msg.data[1] = GbChgM_GetChgEndReason2();

    GbChgCan_Msg[3].Msg.data[2] = GbChgM_GetChgEndReason3();

    GbChgCan_Msg[3].Msg.data[3] = GbChgM_GetChgEndReason4();  
    ...
    GbChgCan_Msg[5].Msg.data[0] = GbChgM_GetChgHandShakeErr();
    GbChgCan_Msg[5].Msg.data[1] = GbChgM_GetChgConfigErr();
    GbChgCan_Msg[5].Msg.data[2] = GbChgM_GetChargingErr();
    GbChgCan_Msg[5].Msg.data[3] = GbChgM_GetChargeEndErr();
}
...
void GbChgCan_TxData(void)
{
     ...
     GbChgCan_Msg[MSG_BRO].Flg = GbChgM_GetFlgBro();  
     GbChgCan_Msg[MSG_BCL].Flg = GbChgM_GetFlgBcl();
     GbChgCan_Msg[MSG_BSM].Flg = GbChgM_GetFlgBsm();
     GbChgCan_Msg[MSG_BST].Flg = GbChgM_GetFlgBst();
     GbChgCan_Msg[MSG_BSD].Flg = GbChgM_GetFlgBsd();
     GbChgCan_Msg[MSG_BEM].Flg = GbChgM_GetFlgBem();
     GbChgCan_Msg[MSG_BHM].Flg = GbChgM_GetFlgBhm();
...
{                                          
   ...
   pGbChgCtrl = &GbChgCan_Ctrl;
    ...
    switch (msg.id)
    {
     case ID_CHM:
    {
        pGbRxData->Chm.MainVersion[0] = msg.data[1];
        pGbRxData->Chm.MainVersion[1] = msg.data[0];
        pGbRxData->Chm.SunVersion = msg.data[2];
    ...
    case ID_CML:    /* maximum output of charger */
    {
        ...
        wtmp = msg.data[7];
        wtmp = (wtmp << 8) + msg.data[6];
        pGbRxData->Cml.MinOutCur = wtmp;
        Chg_UpdateOutPutMinCurrent(wtmp);
    ...
    case ID_CCS:    /* charger status */
    {
        ...
        wtmp = msg.data[6];
        wtmp = (wtmp & 0x0003);
        pGbRxData->Ccs.PermitChg = (uByte)wtmp;
