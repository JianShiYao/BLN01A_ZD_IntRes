...
#define RLY_STA_CLOSE	   0x01    //继电器状态state
...
static void HvM_PowOffChk(sHvM_CtrlHvType *p_ctrl, sHvM_FlagType *p_flg, sHvM_CntType *p_cnt)
{
    ...
    if((Err_GetHwErrLevel(ERR_HW_CAN4) >= 1)
     || (Err_GetBatSysErrLevel(ERR_BATSYS_ISO_ERR) >= 3)
     || (Err_GetBatErrLevel(ERR_BAT_SUMV_HIGH)>= 3)
     || (Err_GetBatErrLevel(ERR_BAT_SUMV_LOW)>= 3)
     || (Err_GetBatErrLevel(ERR_BAT_CELLV_HIGH)>= 3)
     || (Err_GetBatErrLevel(ERR_BAT_CELLV_LOW)>= 3)
     || (Err_GetBatErrLevel(ERR_BAT_T_HIGH)>= 3)
     || (Err_GetBatErrLevel(ERR_BAT_T_LOW)>= 3)     
     || (Err_GetBatErrLevel(ERR_BAT_CUR_HIGH_DCH)>= 3)
...
void HvM_UpdateStep(uByte step)
{
    gby_test =  step;
}
uByte HvM_GetStep(void)
{
    return gby_test; 
}
static void HvM_NFDBKHvCtrl(sHvM_CtrlHvType *p_ctrl, sHvM_FlagType *p_flg, sHvM_CntType *p_cnt, sHvM_ErrType  *p_err)
{
    ...
    HvM_UpdateStep(p_ctrl->Step);
    
    HvM_VcuInfo.Cmd = 0x01;

    if( (FLAG_OK == Chg_GetDcLinkSt())  //国标DC充 ,AC充  铁城
      ||(FLAG_OK == Chg_GetAcLinkSt())) 
      //||(FLAG_OK == flag_onchg_connect)
     
      //充电阶段 互锁
    {
        HvM_CtrlRelayCmd.Bcmd = RELAY_OPEN_CMD;
        return_rslt = Actor_CtrlRelay(CPU_HEAT_ID, HvM_CtrlRelayCmd);
       // return_rslt = Actor_CtrlRelay(POS_RELAY_ID, HvM_CtrlRelayCmd);    //用总正来控制BMU电源
        BatSys_UpdatePreRlySt(0);
       // BatSys_UpdatePosRlySt(0);
        
        
        
        if( //(3 == Err_GetBatSysErrLevel(ERR_BATSYS_ISO_ERR))   //iso err 2
            (Err_GetBatErrLevel(ERR_BAT_SUMV_HIGH) >= 3)   //sumv high 2
          ||(Err_GetBatErrLevel(ERR_BAT_CELLV_HIGH) >= 3)  //cellv high 2
          ||(gby_maxt >= 82) 
          ||(Err_GetBatErrLevel(ERR_BAT_CUR_HIGH_CHG) >= 3))     // cur high 2
        {
            if(HvM_Cnt.ChgRlyOffDelay >= HVM_CNT_5S) 
            {
                HvM_Cnt.ChgRlyOffDelay = 1000;
                gby_chg_relay_off_flag = 1;
                HvM_CtrlRelayCmd.Bcmd = RELAY_OPEN_CMD;
        ...
        else if(gby_chg_relay_off_flag == 0) 
        {
            HvM_Cnt.ChgRlyOffDelay =0;
    ...
    else     //放电阶段
    {
        switch(p_ctrl->Step)
        {
            case PWR_ON_INIT:
            {
                ...
                /* close pre relay, neg relay*/   //第一步：上电收到vcu命令
                if((HvM_VcuInfo.Cmd & 0x03) == 0x01)   //VCU CMD
                {
                    if(HVM_FLAG_FAIL == p_flg->CloseNegRly)
                    {
                        ...
                        HvM_CtrlRelayCmd.Bcmd = RELAY_CLOSE_CMD;
                        return_rslt = Actor_CtrlRelay(NEG_RELAY_ID, HvM_CtrlRelayCmd); //闭合电池箱内的继电器 用FAN来控制
            ...
            case PWR_ON:
            {
                ...
                if((HvM_VcuInfo.Cmd & 0x03) == 0x01)   //VCU CMD
                {
                    if(HVM_FLAG_FAIL == p_flg->StartPreChg)
                    {
                        ...
                        HvM_CtrlRelayCmd.Bcmd = RELAY_CLOSE_CMD;     //
                        return_rslt = Actor_CtrlRelay(CPU_HEAT_ID,HvM_CtrlRelayCmd); //闭合预充   
                    ...
                    if((lwd_sumv_out >= wtmp) && (lwd_sumv_int >= 650)) // the sumv is detectde by vcu 
                    {
            ...
            case PWR_OFF_FST_VCU:     //VCU先发下电命令；
            {
                /* current is safety */
            ...
            case PWR_OFF_FST_BCU:
            {
                ...
                {
                    p_cnt->WaitVcuCmd++;
                    if(p_cnt->WaitVcuCmd > HVM_CNT_2S)
                    {
                        p_cnt->WaitVcuCmd = 0;
                        p_ctrl->Step = PWR_OFF_SND;
                        p_flg->ForbidPowerOn = HVM_FLAG_OK; /* forbid power on again*/
 ...
 void HvM_ModuleInit(void)
{
    ...
    HvM_CtrlRelayCmd.Bcmd = RELAY_CLOSE_CMD;     //用于控制BMU供电电源
    return_rslt = Actor_CtrlRelay(POS_RELAY_ID, HvM_CtrlRelayCmd);
    BatSys_UpdatePosRlySt(1);  
