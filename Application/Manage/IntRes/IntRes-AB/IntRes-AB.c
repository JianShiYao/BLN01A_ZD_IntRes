...
typedef struct
{
    ...
    uWord  LowSumV;
    uWord  HighSumV;
    uWord  LowCellV[20];     //MAX_CELL_NUM
    uWord  HighCellV[20];     //MAX_CELL_NUM
} sIntRes_CalcType;
...
typedef struct
{
    uWord Sum_Intres_T1[5]; // 0.1 mΩ, t <= 10'C
    uWord Sum_Intres_T2[5]; // 0.1 mΩ, t > 10'C

    uByte Max_Intres_T1;   // 0.1 mΩ, t <= 10'C
    uByte Min_Intres_T1;   // 0.1 mΩ, t <= 10'C
    uByte Avg_Intres_T1;   // 0.1 mΩ, t <= 10'C

    uByte Max_Intres_T2;   // 0.1 mΩ, t > 10'C
    uByte Min_Intres_T2;   // 0.1 mΩ, t > 10'C
    uByte Avg_Intres_T2;   // 0.1 mΩ, t > 10'C
} sIntRes_InforType;
...
static void IntRes_CalcProc(void)
{
    ...
    Bat_UpdateCurrent(INSCUR,30000); //调试用，设定固定内电流，内总压，正常开启式屏蔽掉宏
    Bat_UpdateSumV(INT_SUMV,3120);
    wtmp = 3200;
    for(i = 1; i < 37; i++)
    {      
        Bat_UpdateCellV(1,i,wtmp); //37cells  V=3.2,...3.57
        wtmp += 10;
    }
...
/********************************************************************************************************/    
// Wait low cuurent, get initial value of min current and max voltge 
/********************************************************************************************************/
        ...
        if( (current < INTRES_DCHCUR_8A) && (current > INTRES_CHGCUR_1A) )
        {
            if(IntRes_Calc.TimeCnt2 < INTRES_TIMECNT_5S)
            {
                ...
                if(INTRES_TIMECNT_1S == IntRes_Calc.TimeCnt2)
                {
                    ...
                    for(i = 0; i < (bmu_num + 1); i++)
                    {
                        wtmp = Bat_GetPackSerialNum(i);
                        for(j = 0; j < wtmp; j++)
                        {
                            IntRes_Calc.HighCellV[j] = Bat_GetCellV(i, j + 1);
                        }
                    }
                }
                ...
                if(IntRes_Calc.TimeCnt2 > INTRES_TIMECNT_1S)
                {
                    ...
                    for(i = 0; i < (bmu_num + 1); i++)
                    {
                        ...
                        for(j = 0; j < wtmp; j++)
                        {
                            if(Bat_GetCellV(i, j + 1) > IntRes_Calc.HighCellV[j])
                            {
                                IntRes_Calc.HighCellV[j] = Bat_GetCellV(i, j + 1);
                            }
                        }
        ...
        else if(current > INTRES_DCHCUR_60A)
        {
            ...
            else
            {
                ...
                for(i = 0; i < (bmu_num + 1); i++)
                {
                    ...
                    for(j = 0; j < wtmp; j++)
                    {
                        IntRes_Calc.LowCellV[j] = Bat_GetCellV(i, j + 1);
                    }                                                                            
                }
            }
        }
...
/*********************************************************************************************************/
// Caculate IntRes when this process get low current|high voltage and high current|low voltage 
/*********************************************************************************************************/        
            ...
            if (IntRes_Calc.TimeCnt2<INTRES_TIMECNT_11S)   
            {
                ...
                if(IntRes_Calc.TimeCnt2 < INTRES_TIMECNT_10S)	
                {
                    ...
                    for(i = 0; i < (bmu_num + 1); i++)
                    {
                        ...
                        for(j = 0; j < wtmp; j++)
                        {
                            if(Bat_GetCellV(i, j + 1) < IntRes_Calc.LowCellV[j])
                            {
                                IntRes_Calc.LowCellV[j] = Bat_GetCellV(i, j + 1);
                            }
                        }
            ...
            if(IntRes_Calc.TimeCnt2 < INTRES_TIMECNT_11S)
            {
                ...
                if(IntRes_Calc.HighCur > IntRes_Calc.LowCur)
                {
                    if(IntRes_Calc.HighSumV > IntRes_Calc.LowSumV)
                    {
                        ...
                        intres = (uWord)(lwtmp / (IntRes_Calc.HighCur - IntRes_Calc.LowCur));		//0.1mΩ unit
                    ...
                    for(i = 0; i < (bmu_num + 1); i++)
                    {
                        ...
                        for(j = 0; j < wtmp; j++)
                        {
                            if(IntRes_Calc.HighCellV[j] > IntRes_Calc.LowCellV[j])
                            {
                                lwtmp = IntRes_Calc.HighCellV[j]	-	IntRes_Calc.LowCellV[j];
                                lwtmp *=1000;
                                intres = (uWord)(lwtmp / (IntRes_Calc.HighCur - IntRes_Calc.LowCur));		//0.01mΩ unit

#ifdef BMS_ENABLE_CALC_INTRES

                                (void)Bat_UpdateCellR(bmu_num, (j + 1), intres);
