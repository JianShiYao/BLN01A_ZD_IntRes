...
const sVersion_CodeType Sof_VerDef =
{
    MODULE_SOF_NAME,
    SOF_SW_MAJOR_VERSION,
    SOF_SW_MINOR_VERSION,
    SOF_SW_REVISION_VERSION,
    0,
    0,
    SOF_DEVELOP_STATUS
};
...
ModuleID_Type SOF_A_GetVesionModuleVer(sVersion_CodeType * VerPtr)
{      
    *VerPtr = Sof_VerDef;
    return MODULE_SOF_ID;   
}
...
static void SOF_CalcPmtCur(sSOF_CalcValueType *Calc_ptr)
{
    ...
    if(SOF_FUN_ENABLE == SOF_Macro.Sof_Pulse_Cur_Modify)
    {
        ...
        if(lwtmp > OFFSET_CUR)
        {
            ...
            if(gby_PulseDisCurCnt >= SOF_PULSE_DISCHARGE_CUR_TIME)
            {
                gby_PulseDisCurCnt = SOF_PULSE_DISCHARGE_CUR_TIME;
        ...
        else if(lwtmp < OFFSET_CUR)
        {
            ...
            if(gby_PulseFeedCurCnt >= SOF_PULSE_FEEDBACK_CUR_TIME)
            {
                gby_PulseFeedCurCnt = SOF_PULSE_FEEDBACK_CUR_TIME;
