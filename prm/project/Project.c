...
/*void Prj_WrCapacity(void) 
{
    ...
    *((uLWord *)&value_array[0]) = 100000; //RatedCap
    
    *((uLWord *)&value_array[4]) = 100000; //Actual Cap
    
    (void)SysConf_UpdataParaToRam(SYSTEMST_RATED_ACTUAL_CAPACITY,value_array);
}*/
...
void Prj_CheckAllCellV(void)
{
    ...
    if(bmu_no > 1)
    {
        ...
        btmp = (uByte)Bat_GetPackSerialNum(bmu_no);
        if(btmp > 0)
        {

            for(i = 0; i < bmu_no; i++)
            {
                if((Bat_GetCellV(bmu_no,1) == 0) && (Bat_GetCellV(bmu_no, btmp -1) == 0))
                {
                    break;
                }
            }
    ...
    {
        btmp = (uByte)Bat_GetPackSerialNum(0);
        {
            // first cellv and last cellv have receive
            if((Bat_GetCellV(0,1) > 0) && (Bat_GetCellV(0, btmp - 1) > 0))
            {
                gby_ReadyFlag |= ALL_CELLV_READY;
            }
        }        
...
void Prj_CheckAllTemper(void)
{
    ...
    if(bmu_no > 1)
    {
        ...
        btmp = Bat_GetPackTemperNum(bmu_no);
        if(btmp > 0)
        {

            for(i = 0; i < bmu_no; i++)
            {
                if((Bat_GetTemper(bmu_no,1) == 0) && (Bat_GetTemper(bmu_no, btmp -1) == 0))
                {
                    break;
                }
            }
    ...
    {
        btmp = Bat_GetPackTemperNum(0);
        if((Bat_GetTemper(0,1) > 0) && (Bat_GetTemper(0, btmp - 1) > 0))
        {
            gby_ReadyFlag |= ALL_TEMPER_READY;
        }        
    }
